 ...
 * @author junedkazi
 */
@WebServlet(name = "updateraheel", urlPatterns = {"/updateraheel"})
public class updateraheel extends HttpServlet {
...
for(int i=0;i<song.length;i++)
    {
      ...
      rs1=m.Search(song[i]);
        ...
        if(rs1.next())
        {
            String get_val=request.getParameter("test_text"+i);
            int act_price=Integer.parseInt(get_val)*Integer.parseInt(rs1.getString("price"));
            String imid=rs1.getString("image_id");
        out.println("<td valign=center><img src=img/"+imid+".jpg height=75 width=75>");
       out.println("<td>"+get_val+"&nbsp</td> <td>&nbsp"
                + Integer.parseInt(rs1.getString("price"))+"</td>&nbsp"+"<td>"+act_price+"</td></tr>");
 ...
 out.println("<form name=passkey action=verifypasskey method=post>");
            ...
            Logger.getLogger(updateraheel.class.getName()).log(Level.SEVERE, null, ex);
        } finally {            
            out.close();
        }
    }
